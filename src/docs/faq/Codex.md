...
### ::ic:round-whatshot:: 为什么选定了模型，账单里还会看到别的模型调用

很多人在 `config.toml` 或 `/model` 里明明指定了某个模型（比如 `gpt-5.4`），却发现 CLI 输出、或者中转站的用量记录里还是多出了别的模型调用，误以为是配置错了、或者账号被盗用——**这是 Codex 的正常设计，不是异常**。

::: important
Codex CLI / VSCode 插件会在一些"辅助场景"里自动调用模型，这些调用和你在 `/model` 或 `config.toml` 里指定的主模型无关：

- **自动生成会话标题**：新建一个会话并发送第一条消息之后，Codex 会自动调用模型生成这个会话的名称，也就是 `/resume` 历史会话列表里显示的那一行标题。这个标题生成通常用更轻量的模型完成，和你实际对话使用的模型是两回事
- **`/compact` 压缩上下文**：当对话历史过长执行 `/compact` 时，生成压缩摘要的这一步，也可能由内部机制单独完成，不完全等同于你选定的主模型在处理
- **`/review` 代码审查**：执行 `/review` 命令时，Codex 生成的审查报告也可能走独立的调用流程
- **`web_search_request` 等内置功能**：开启内置网络搜索等辅助能力后，检索、整理搜索结果的中间步骤同样可能产生额外调用

这些都是 Codex 客户端/CLI 官方自带的行为，无法通过 `config.toml` 里的配置项完全关闭。
:::

这些辅助调用消耗的 Token 通常很少，属于官方设计的正常现象，看到账单里出现你没有主动选择的模型，不代表配置出错，也不代表账号异常，无需担心。

::: tip 如何核实这笔调用
如果想确认这笔调用具体是什么模型、花了多少钱，可以登录 PackyApi 控制台，进入 **消费日志** 页面，按时间线找到对应的那一笔记录，里面会明确标出实际调用的模型名称，可以据此和上面的场景对照核实，不用只凭猜测。
:::
