...
::: important 模型填写说明
如果你的 DeepSeek 分组开启了 `1m` 上下文，请使用 `deepseek-v4-pro[1m]`；如果没有开启，则使用 `deepseek-v4-pro`。
...
::: warning
手动配置时请保持模型名称与令牌分组实际支持的模型一致。如果没有开启 `1m` 上下文，需要将上方的 `deepseek-v4-pro[1m]` 改为 `deepseek-v4-pro`。
:::
...
2. 在 Claude Code 界面确认左侧显示的模型名称是否为刚才配置的 DeepSeek 模型，例如 `deepseek-v4-pro[1m]`。
